"""Sparkov-specific normalization into the canonical FraudLens raw schema."""
...
def _canonicalize_sparkov_columns(
    df: DataFrame,
    fraud_label_value: str,
    non_fraud_label_value: str,
) -> DataFrame:
...
def normalize_sparkov_raw(
    ...
    df_canonical = _canonicalize_sparkov_columns(
        df_source,
        fraud_label_value,
        non_fraud_label_value,
    )
    df_canonical.write.mode("overwrite").json(output_path)
    return df_canonical


run = normalize_sparkov_raw
run_sparkov_ingest = normalize_sparkov_raw
